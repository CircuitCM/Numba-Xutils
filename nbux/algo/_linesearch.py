...
# the precision of the function output. However because all of these methods utilize some form of bisection fallback, we
# at least have a guarantee it will reach a stationary point. Depending on the optimizer there is some risk of premature
# stopping, but that should be extremely rare. That also means er_tol ~ root_point*(float64 epsilon). May implement a
# more refined stopping api in the future.

# nbu is my lib that extends numba in helpful ways.
# math native calls almost always have faster implements than the numpy equivalent, for numba scalar ops. eg
# mt.copysign. type casting not difference tho.
...
# register jittable with inlining, in interpreter runs as py func, but compiles
# with inlining when referenced in jit decorated scope.
@nbu.rgi
def _bracketed_newton(fd_op, lo, hi, er_tol: float = 1e-14, max_iters: int = 20, sign: int = 1):
    """
    ...
    :param fd_op: Can be a function or a function operator (tuple). It recieves a single scalar value for the point
    estimate.
    :param sign: =1 we expect to have f(lo)<f(root)<f(hi). if -1 we expect f(lo)>f(root)>f(hi). If this expectation is
    unknown, it controls the bracketing bias eg if f is all positives then the bracket will halve from left to right
    until reaching hi, if negatives vice versa. Note: both sides wrong could make convergence messy. However it can
    still work with a high likelihood if we only know the sign of either lo or hi, and that our target root has a root
    on the opposite sign side.
    :param lo: Lower bound.
    :param hi: Upper bound.
    :param er_tol: Step-size tolerance.
    :param max_iters: Iteration limit.
    ...
    lam = 0.5 * (lo + hi)
    while ict > 0:
        # `fd_op` can be a function or tuple(callable, *args). op_call_args
...
@nbu.jt
def _bracketed_secant(f_op, lo, hi, er_tol: float = 1e-14, max_iters: int = 20, sign: int = 1):
    """
    An older version of signedroot_secant, keeping around as it's simple.
    Basically the same thing as bracketed newton. f_op returns f. However the secant points use the two most recent
    points, instead of the updated lo hi brackets, this typically gets the most out of the secant method, while still
    guaranteeing convergence with bisection bracketing, we could make it more greedy as well by using increments >1/2 or
    <1/2.

    Variable calculations are all f64.

    :param f_op: Can be a function or a function operator (tuple). It receives a single scalar value for the point
    estimate.
    :param sign: =1 we expect to have f(lo)<f(root)<f(hi). if -1 we expect f(lo)>f(root)>f(hi). If this expectation is
    unknown, it controls the bracketing bias eg if f is all positives and sign=1, then the bracket will halve from right
    to left until reaching hi, if negatives and sign=1 then left to right. Note: If both sides are wrong then
    convergence is impossible. However, it still converges with a high likelihood if we only know the sign of either lo
    or hi, and that our target root has a root on the opposite sign side.
    :param lo: Lower bound.
    :param hi: Upper bound.
    :param er_tol: Step-size tolerance.
    :param max_iters: Iteration limit.

    Other Notes: Convergence is only guaranteed when there is a single root within the bracket. In a two root scenario
    because of how bracketing is handled, this method will always converge to the root with sign(slope)=sign (if it
    converges at all). Sign=-1: left root, =1: right root. Brent's method doesn't have this issue but is significantly
    slower. See alternative methods that can deal with multiple roots.
...
def signedroot_secant(
    ...
    is still very high due to the initial side rejection strategy explained above, by decreasing the bracketing
    increment to a range that guarantees sampling a basin br_rate <.5, you once more recover guaranteed
    convergence to the signed root.

    Variable calculations are all f64.

    :param f_op: Can be a function or a function operator (tuple) that includes its arguments. It receives a single
    scalar value for the point estimate.
    :param lo: Starting lower bound.
    :param hi: Starting upper bound.
    :param br_rate: (0,1). The bracket increment, at .5 it's classic bisection, less than .5 it will bracket more
    conservatively, greater than .5 it's more eager.
    :param max_iters: The number of function evaluations before the process is forced to terminate prematurely and
    ...
    :param eager: While the opposite boundary hasn't been found do we bisect or still use secant? If the opposite bound
    is not yet located a secant step might push the next point towards the wrong root initially, this could delay root
    convergence slightly at first, but it will still converge correctly.
    :param fallb: A secant specific parameter to fall back to interpolation using the opposite boundary.
    :param br_tol: Bracket fallback tolerance, floating point precision tolerance before the secant interpolation is
    considered unreliable. If the difference between secant points is less than this we consider it unreliable and
    fallback to bisection.
    :param er_tol: If the distance between the most recent two points is less than er_tol, the iterations are considered
    complete and function returns. Two most recent points are used instead of the lower and upper bound as it better
    reflects the progress of the secant steps.
    :param rel_err: Is bracket fallback tolerance based on value relative error or absolute value. Typically value
    relative is correct unless you know there is a constant error rate in value measurements (e.g. noise or discrete
    surface).
    :param dtyp: The data type that the root search should operate with. This will convert all computations and values
    to this datatype. If left as None, the datatype will be gathered from the input type of lo.
    ...
    # We init op_bracket by checking if the unknown point is negative, for this algo we assume that we know either lo or
    # hi always has a positive sign for the general problem, my choice was due to the typical format of boundary
    # solutions. If we seek a negative sloped root then we know our left side is positive, but the right side may have a
    # basin or multiple roots (positive or negative areas), therefore we check if our right side has a negative
    # bracketing location.
    ...
    lamalt = lam - _1
    # If first secant step is requested on iteration one, this value forces the
    # same initial bracket behavior.
    falt = f
    ict = int(max_iters)
    while ict > 0:
        lamb = lo * lrt + hi * hrt
...
def signedroot_newton(
    ...
    Strategy mirrors the secant variant:
      ...
      * We only accept Newton steps that land in an admissible sub-interval (or the full bracket once op_bracket is
      True).
      * Before a negative-slope target (sign == -1) we flip signs so the effective problem is always increasing in the
      bracket:
    ...
    Notes:
      ...
      * Convergence is only guaranteed when there is a single congruent-slope root in the bracket (same caveats as the
      secant version).
...
def signseeking_halley(
    ...
    A bracketed halley method that exploits prior knowledge about the side of the root and the desired root slope sign.
      ...
      Strategy mirrors the secant variant:
        ...
        * We only accept Newton steps that land in an admissible sub-interval (or the full bracket once op_bracket is
        True).
        * Before a negative-slope target (sign == -1) we flip relations so the effective problem is always increasing in
        the bracket:
